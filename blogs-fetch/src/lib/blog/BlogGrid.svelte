<script>
	...
	blogs.forEach((blog) => {
		let sports = blog.sport;
		sports.forEach((sport) => {
			let matchingSport = sortedBlogs.filter(
				(categories) => categories.category.toLowerCase() === sport.toLowerCase()
			);
			if (!matchingSport[0].blogs.includes(blog)) {
				matchingSport[0].blogs.push(blog);
			}
		});
		if (blog.isFeaturedOnMain) {
			featuredBlogs.push(blog);
		}
	});

	sortedBlogs.forEach((sport) => {
		sport.blogs = sport.blogs.sort((blogOne, blogTwo) => new Date(blogTwo.date) - new Date(blogOne.date));
	});
						...
						<div class="featured-item swiper-slide" data-sport={featuredBlog.sport}>
							...
							<div class="featured-content">
								...
								<div class="author-details">
									<img
										src={`/assets/images/coaches/${featuredBlog.author
											.replace(/\s+/g, '-')
											.toLowerCase()}.png`}
										alt="Coach Thumbnail"
									...
									<div>
										...
										{:else}
										<p class="muted capitalize">{featuredBlog.sport[0]}</p>
										{/if}
									</div>
								</div>

								<div class="featured-bottom">
									<a href={`blog/${featuredBlog.sport[0].replace(' ', '-')}/${featuredBlog.slug}`}>
