<script>
	...
	function checkPremium(event, blog) {
		if (blog.isPremium && !$premiumUser && blog.layout != 'video' && !$page.url.search.includes('premium=true')) {
			...
			if (sport === 'all') {
				...
				slug = `/blog-inner/${sport.replace(/\s+/g, '-').toLowerCase()}/${blog.slug}`;	
			}
			goto(slug);
		}
	}
	...
	let authorName = '';
	if (blog.categories.filter(e => e['article_category_id']['title'] === 'Newsletter').length > 0) {
		authorName = 'Blayze Newsletter';
	} else {
		authorName = blog.authors['0']['author_id']['full_name'];
	}

	let authorSlug = authorName.replace(/\s+/g, '-').toLowerCase();	
	
</script>

{#if blog.tag != 'mazda'}
	<a class="blog-card" class:swiper-slide={slider === 'true'} href={`/blog-inner/${sport.replace(/\s+/g, '-').toLowerCase()}/${blog.slug}`} sveltekit:prefetch>
		...
		<div class="blog">
			<div class="relative">
				...
				{/if}
				<img data-src={`https://cms.blayze-cdn.io/${blog['hero_image']['filename_disk']}`} alt="Blog Alt Tag" class="blog-img lozad" />
			</div>

			<div class="content">
				<h2 class="headline sm">{blog.title}</h2>
				...
				<div class="card-bottom">
					<div class="author-details">
						<img
							data-src={`/assets/images/coaches/${authorSlug}.png`}
							alt="Coach Thumbnail"
							class="coach-thumbnail lozad"
						/>
						<div>
