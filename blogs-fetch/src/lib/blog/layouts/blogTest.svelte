<script>
	...
	function loadNewsletters() {
		console.log( blog['categories'].some(category => category['article_category_id']['title'] === 'Newsletter' ), newsletterID );
		if (blog['categories'].some(category => category['article_category_id']['title'] === 'Newsletter' ) && newsletterID) {
			document.querySelector(`._form_${newsletterID}`).innerHTML = '';
		...
		if (blog['categories'].some(category => category['article_category_id']['title'] === 'Newsletter' ) && hasTwitterEmbed) {
			new Promise((resolve, reject) => {
				const script = document.createElement('script');
...
{#if !plusDialogRevealed}
	<div class="blog-hero">
		...
		<div class="hero-bottom">
			<div class="author-details">
				<img src={`/assets/images/coaches/${author.replace(/\s+/g, '-').toLowerCase()}.png`} alt="Coach Thumbnail" class="coach-thumbnail" id={authorSlug} />
				<div>
					<p><strong>{author}</strong></p>
					<p class="muted capitalize">{sport}</p>
				</div>
			</div>

			<div class="social-media-icons">
				<a target="_blank" href={`https://facebook.com/sharer/sharer.php?u=https://blayze.io/blog/${slug}/&quote=${meta_description}`} >
					<img src="/assets/icons/facebook.svg" alt="Share on Facebook" />
				</a>
				<a target="_blank" href={`https://twitter.com/intent/tweet?url=https://blayze.io/blog/${slug}&text=${meta_description}`} >
					<img src="/assets/icons/twitter.svg" alt="Share on Twitter" />
				</a>
				<a target="_blank" href={`https://www.pinterest.com/pin/create/button/?url=https://blayze.io/blog/${slug}/&media=https://blayze.io/assets${featured_image}&description=${meta_description}`} >
		...
		{#if category != 'newsletter'}
			<img src={`https://cms.blayze-cdn.io/${featured_image}`} alt={`${title} Image`} class="blog-img" />
		{/if}
	</div>
	...
	{#if !author.startsWith('Blayze')}
		<div class="author-banner" bind:this={authorBanner}>
			<div class="container">
				<div class="author-details">
					<img src={`/assets/images/coaches/${author.replace(/\s+/g, '-').toLowerCase()}.png`} alt="Coach Thumbnail" class="coach-thumbnail" />
					<div>
				...
				<div class="buttons">
					...
					<div class="close-button" on:click={() => authorBanner.classList.add('out-of-view')}>
						<svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" >
							<path d="M0 24C0 10.7452 10.7452 0 24 0V0C37.2548 0 48 10.7452 48 24V24C48 37.2548 37.2548 48 24 48V48C10.7452 48 0 37.2548 0 24V24Z" fill="#BDBDBD" />
	...
	{/if}
	<RelatedBlogs title="Related articles" sport={sport} {category} length="4" />
{:else}
