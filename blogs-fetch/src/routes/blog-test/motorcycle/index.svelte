<script context="module">
	export async function load() {
		const blogsFetch = await fetch('http://localhost:5173/api/motorcycle.json');
...
<svelte:head>
	<title>Blayze Motorcycle Racing | Articles</title>
	<meta name="description" content="Become a faster and safer motorcycle rider on the race track through the articles from the world-class team of motorcycle racing coaches at Blayze." />
	<meta property="og:image" content="https://blayze.io/assets/images/meta-image.jpg" />
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0" />
</svelte:head>

<SportSelection bind:value={searchTerm} selectedSport='Motorcycle' />
...
{#if searchTerm.length > 0}
	<section class="container searched">
		...
		<div class="searched-blogs-grid">
			{#each searchedBlogs as metadata}
				<SearchedBlogCard blog={metadata} featured="false" sport='motorcycle' />
			{/each}
		</div>
	</section>
	{:else}
							...
							<div class="featured-content">
								...
								<div class="author-details">
									...
									<div>
										...
										<p class="muted capitalize">Motorcycle</p>
									</div>
								</div>

								<div class="featured-bottom">
									<a href={`/blog-inner/motorcycle/${featuredBlog.slug}`}>
										<div class="cta-button">Read Article</div>
									</a>

									<div class="social-media-icons">
										<a target="_blank" href={`https://facebook.com/sharer/sharer.php?u=https://blayze.io/blog/motorcycle/${featuredBlog.slug}/&quote=${featuredBlog.meta_description}`}>
											<img src="/assets/icons/facebook.svg" alt="Share on Facebook" />
										</a>
										<a target="_blank" href={`https://twitter.com/intent/tweet?url=https://blayze.io/blog/motorcycle/${featuredBlog.slug}&text=${featuredBlog.meta_description}`}>
											<img src="/assets/icons/twitter.svg" alt="Share on Twitter" />
										</a>
										<a target="_blank" href={`https://www.pinterest.com/pin/create/button/?url=https://blayze.io/blog/motorcycle/${featuredBlog.slug}/&media=https://blayze.io/assets${featuredBlog.featured_image}&description=${featuredBlog.meta_description}`}>
	...
	<section class="container pb">
		<div class="position-selection">
			<h2 class="headline">Motorcycle {currentContentType.slice(0, -1).toLowerCase()} categories</h2>
				...
				<div class="blogs-box container">
					...
					<div class="blog-grid">
						<div class="blogs filtered">
							{#each blogs as blog}
								<BlogCard blog={blog} featured="false" sport='motorcycle' />
							{/each}
						</div>
