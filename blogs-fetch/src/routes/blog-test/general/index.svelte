<script context="module">
	export const load = async () => {
		...
		const blogsFetch = await fetch('http://localhost:5173/api/test.json');
...
<script>
	...
	function sortBlogs() {
		...
		sortedByDate.forEach((blog) => {
			...
			console.log('matching: ', matchingTypes);
			if (blog['is_featured']) {
...
<svelte:head>
	<title>Blayze General | Articles</title>
	<meta name="description" content="Articles from the team at Blayze. Find racing and driving tips perfect for HPDE, club racing, autocross, sim racing, or any type of racing." />
	<meta property="og:image" content="https://blayze.io/assets/images/meta-image.jpg" />
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0" />
</svelte:head>

<SportSelection bind:value={searchTerm} selectedSport='' />
